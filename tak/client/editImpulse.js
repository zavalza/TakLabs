Template.editImpulse.rendered = function()
{
  Meteor.call('getImpulseDoc', Session.get('currentImpulse'), function(error, result){
    if(!error)
    {
      Session.set('selectedTags', result.person_tags);
      var types = document.getElementsByName('impulseType');
      for(var i = 0; i < types.length; i++)
      {
        if(result.tag_ids.indexOf(types[i].value) != -1)
        {
      ...
      {
        if(result.remote == remotes[i].value)
        {
          remotes[i].checked = true;
        }
      }
      ...
      {
        if(result.tag_ids.indexOf(rewards[i].id) != -1)
        {
...
Template.editImpulse.helpers({
  ...
  {
    return Impulses.find({_id:Session.get('currentImpulse')});
  },
  tagOptions : function()
    {
